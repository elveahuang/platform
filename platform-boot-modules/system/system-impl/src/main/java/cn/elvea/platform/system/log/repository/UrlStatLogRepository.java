...
import cn.elvea.platform.commons.core.log.domain.UrlStatLogDto;
...
public interface UrlStatLogRepository extends BaseEntityRepository<UrlStatLogEntity, Long> {
    ...
    @Query("""
            SELECT new cn.elvea.platform.commons.core.log.domain.UrlStatLogDto(t.path, avg(t.execTime), max(t.execTime), min(t.execTime), sum(t.execTime), count(t.id))
            FROM UrlLogEntity t
            where t.path = :path
            """)
